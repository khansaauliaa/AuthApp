...
import com.khansafzh.authapp.utils.Conts.setTimeStamp
...
class Add : AppCompatActivity() {
    ...
    private fun setUpView() {
        if (isEdit){
            btn_save.text = getString(R.string.txt_update)
            et_name_add.text = Editable.Factory.getInstance().newEditable(recipes?.recipeName)
    ...
    private fun createRecipe(recipeId: String?) : Task<Void> {
        ...
        val name = et_name_add.text.toString()
